...
Die Anwendung kann auch direkt aus dem Quellcode gebaut werden.
...
Anschließend kann ein lokales Docker-Image erstellt werden:

```bash
docker build -t devops-api-image:local .
```

Der Container kann danach gestartet werden:

```bash
docker run -d -p 8080:8080 --name devops-api-local devops-api-image:local
```
...
```bash
docker stop devops-api-local
```

Container entfernen:

```bash
docker rm devops-api-local
```
...
Das Projekt kann beispielsweise mit IntelliJ IDEA oder Visual Studio Code gestartet werden.

Vorgehen:

1. Repository klonen.
2. Projekt in der IDE öffnen.
3. Java 17 als JDK auswählen.
4. Maven-Abhängigkeiten laden lassen.
5. Die Spring-Boot-Hauptklasse `DemoApplication.java` öffnen.
6. Die `main`-Methode starten.

Alternativ kann die Anwendung über den Maven Wrapper gestartet werden.
